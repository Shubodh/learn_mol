...
def slice_edge_type_from_edge_feats(edge_feats):
    ...
    """
    edge_types_one_hot = edge_feats[:, :len(SUPPORTED_EDGES)]
...
def calculate_node_edge_pair_loss(node_tar, edge_tar, node_pred, edge_pred):
    ...
    for edge in range(len(SUPPORTED_EDGES)):
        node_edge_preds[:, :, edge] = torch.matmul(edge_pred_3d[:, :, edge], node_pred[:, :len(SUPPORTED_ATOMS)])
    ...
    for edge in range(len(SUPPORTED_EDGES)):
        node_edge_node_preds[:, :, edge] = torch.matmul(node_edge_preds[:, :, edge], node_pred[:, :len(SUPPORTED_ATOMS)].t())
...
def approximate_recon_loss(node_targets, node_preds, triu_targets, triu_preds):
    ...
    node_preds_reduced = torch.sum(node_preds_matrix[:, :len(SUPPORTED_ATOMS)], 0)
